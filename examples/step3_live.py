"""PLAN.md step 3 live-run script.

Drives `RecordingJuju` against a real LXD juju model:

  * `juju add-model jtr-step3` is assumed to exist already (see PLAN.md
    step 3 prereqs).
...
def main() -> int:
    ...
    # PLAN.md step 2 §"stable key ordering". Don't be fooled by the `.jsonl`
    # filename extension.
