...
| `to` | args | string, optional | `mode: "relative"` only. Placement directive(s) from `juju add-unit --to`, forwarded verbatim (comma-joined string, matching jubilant's own `add_unit(to=...)` handling) to `Juju.add_unit(app, num_units=units, to=to)`. Currently only ever set by the CLI/shim translation path (`cli_translate._classify_add_unit`) — RPC-sourced `Application.AddUnits` events never carry this field. Ignored (never present) when `mode: "absolute"`, since `scale-application` has no `--to` flag. |
| `attach_storage` | args | string, optional | `mode: "relative"` only. From `juju add-unit --attach-storage`, forwarded verbatim to `Juju.add_unit(app, num_units=units, attach_storage=attach_storage)`. Same CLI-only provenance and `mode` restriction as `to` above. |
